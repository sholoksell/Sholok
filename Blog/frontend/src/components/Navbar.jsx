...
export default function Navbar() {
  ...
  const { t, language, toggleLanguage, getLocalizedField } = useLanguage();
  ...
  useEffect(() => {
    api.get('/categories').then((r) => {
      setCategories((r.data.categories || []).filter((c) => !c.parentId).map((c) => ({
        slug: c.slug, icon: c.icon || '📝', name: getLocalizedField(c, 'name'),
      })));
    }).catch(() => {});
  }, [language]);
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden py-1">
                      ...
                      {[
                        ...
                        ...(isAdmin ? [{ icon: FiSettings, label: t('adminPanel'), to: '/admin', external: true }] : []),
                      ].map((item) => (
                        item.external ? (
