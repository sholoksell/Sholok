...
const GRADIENT_MAP = {
  ...
  'hobbies-travel': 'from-blue-500 to-cyan-500',
  hobbies: 'from-blue-500 to-cyan-500',
...
export default function Home() {
  ...
  const fetchData = async () => {
    try {
      const [catRes, settingsRes, featuredRes, trendingRes, latestRes, bloggersRes] = await Promise.all([
        api.get('/categories'),
      ...
      setCategories((catRes.data.categories || []).filter((c) => !c.parentId));
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {(categories.length > 0 ? categories : []).map((cat) => {
            const grad = GRADIENT_MAP[cat.slug] || GRADIENT_MAP[cat.group] || 'from-gray-500 to-gray-700';
            return (
              <Link key={cat.slug} to={`/category/${cat.slug}`}
                className={`relative overflow-hidden rounded-2xl p-6 bg-gradient-to-br ${grad} text-white group hover:scale-105 transition-transform duration-300 shadow-lg`}>
                <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -translate-x-4 -translate-y-8" />
                <span className="text-4xl mb-3 block">{cat.icon || '📝'}</span>
                <h3 className="font-bold font-heading text-lg leading-tight">{getLocalizedField(cat, 'name')}</h3>
