...
const { sendMail, renderOrderConfirmationHtml } = require('../lib/mailer');
...
router.post('/orders', customerAuthMiddleware, async (req, res) => {
  try {
    ...
    // Fire-and-forget order confirmation email (don't block response on email)
    (async () => {
      try {
        const customerDoc = await Customer.findById(req.customer.id).select('name email');
        if (customerDoc?.email) {
          await sendMail({
            to: customerDoc.email,
            subject: `Order Confirmation - ${savedOrder.orderNumber}`,
            html: renderOrderConfirmationHtml({ customerName: customerDoc.name, order: savedOrder }),
          });
        }
      } catch (mailErr) {
