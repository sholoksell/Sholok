...
export interface Product {
  id: string;
  // Multilingual: { en: 'Laptop', bn: 'ল্যাপটপ' }
...
const normaliseI18n = (
  ...
  if (nested && typeof nested === 'object') {
    return { en: nested.en || legacyEn || '', bn: nested.bn || legacyBn || '' };
  }
  if (typeof nested === 'string') return { en: nested, bn: legacyBn || '' };
  return { en: legacyEn || '', bn: legacyBn || '' };
};

const mapApiProduct = (apiProd: ApiProduct): Product => ({
  id: apiProd._id,
  name: normaliseI18n(apiProd.name as any, undefined, (apiProd as any).nameBn),
  slug: apiProd.slug,
  description: normaliseI18n(apiProd.description as any, undefined, (apiProd as any).descriptionBn),
  shortDescription: normaliseI18n(apiProd.shortDescription as any, undefined, (apiProd as any).shortDescriptionBn),
  ...
  videoUrl: '',
  tags: apiProd.tags,
  metaTitle: typeof apiProd.name === 'string' ? apiProd.name : (apiProd.name as any)?.en || '',
  metaDescription: typeof apiProd.shortDescription === 'string' ? apiProd.shortDescription : '',
