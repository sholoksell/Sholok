  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      ...
      const productName = typeof p.name === 'object'
        ? `${p.name.en || ''} ${p.name.bn || ''}`.toLowerCase()
        ...
        <Card className="border-destructive/50 bg-destructive/5">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <p className="text-sm text-foreground">
              ...
              {lowStockProducts.slice(0, 3).map((p) => translateField(p.name, language)).join(', ')}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-foreground">{translateField(product.name, language)}</p>
