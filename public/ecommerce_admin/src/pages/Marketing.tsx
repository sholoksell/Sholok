...
export default function Marketing() {
  ...
  const [bannerForm, setBannerForm] = useState({ title: '', titleBn: '', description: '', descriptionBn: '', image: '', link: '', placement: 'homepage_slider' as Banner['placement'], isActive: true, priority: 0 });
  ...
  const openNewBanner = () => {
    ...
    setBannerForm({ title: '', titleBn: '', description: '', descriptionBn: '', image: '', link: '', placement: 'homepage_slider', isActive: true, priority: 0 });
    setBannerDialogOpen(true);
  };

  const openEditBanner = (b: Banner) => {
    setEditingBanner(b);
    setBannerForm({ title: b.title, titleBn: b.titleBn || '', description: b.description || '', descriptionBn: b.descriptionBn || '', image: b.image, link: b.link || '', placement: b.placement, isActive: b.isActive, priority: b.priority });
    setBannerDialogOpen(true);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-3xl font-bold text-foreground">{t('marketing')}</h1>
      ...
      {tab === 'banners' && (
        <Card className="glass-card border-border">
          <CardHeader><div className="flex items-center justify-between"><CardTitle>Banners</CardTitle><Button onClick={openNewBanner}><Plus className="w-4 h-4 mr-2" />{t('addBanner')}</Button></div></CardHeader>
          <CardContent>
            <Table>
      ...
      <Dialog open={bannerDialogOpen} onOpenChange={setBannerDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><label className="text-sm font-medium">Title (English)</label><Input value={bannerForm.title} onChange={e => setBannerForm({...bannerForm, title: e.target.value})} placeholder="Banner title" /></div>
            <div><label className="text-sm font-medium">শিরোনাম (বাংলা)</label><Input value={bannerForm.titleBn} onChange={e => setBannerForm({...bannerForm, titleBn: e.target.value})} placeholder="বাংলায় ব্যানার শিরোনাম" className="font-bengali" /></div>
            <div><label className="text-sm font-medium">Description (English)</label><Input value={bannerForm.description} onChange={e => setBannerForm({...bannerForm, description: e.target.value})} placeholder="Optional description" /></div>
            <div><label className="text-sm font-medium">বিবরণ (বাংলা)</label><Input value={bannerForm.descriptionBn} onChange={e => setBannerForm({...bannerForm, descriptionBn: e.target.value})} placeholder="বাংলায় বিবরণ (ঐচ্ছিক)" className="font-bengali" /></div>
