 ...
 *
 * Supports:
 *  ① Nested object  { en: "...", bn: "..." }  ← new DB format
 *  ② Plain string                              ← legacy / fallback
 *  ③ null / undefined                         ← returns ''
 *
 * Fallback chain: requested lang → bn → en → ''
 */
...
/**
 * Legacy flat-field resolver (kept for backward compat).
 * Reads `fieldNameBn` for BN, `fieldName` for EN.
 * Prefer translateField() for new code.
 */
export function getLocalizedField<T extends Record<string, any>>(
  ...
  if (!obj) return '';
  // If the field is already a nested multilingual object, delegate to translateField
  const raw = obj[fieldName];
  if (raw && typeof raw === 'object' && ('en' in raw || 'bn' in raw)) {
    return translateField(raw, language);
  }
  if (language === 'bn') {
    return obj[`${fieldName}Bn`] || obj[fieldName] || '';
  }
  return obj[fieldName] || obj[`${fieldName}Bn`] || '';
}
