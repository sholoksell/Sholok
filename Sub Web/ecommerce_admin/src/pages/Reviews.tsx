...
export default function Reviews() {
  ...
  const handleBulkAction = async (action: string) => {
    if (selectedIds.length === 0) return toast.error(t('selectReviewsFirst'));
    try {
      if (action === 'delete') {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">{t('reviewsRatings')}</h1>
        <p className="text-muted-foreground">{t('manageReviewsDesc')}</p>
      </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{Number(stats.averageRating).toFixed(1)}</p>
                </div>
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
          ...
          <Button size="sm" variant="outline" onClick={() => handleBulkAction('approved')}>
            <ThumbsUp className="w-4 h-4 mr-1" /> {t('approve')}
          </Button>
          <Button size="sm" variant="outline" onClick={() => handleBulkAction('rejected')}>
            <ThumbsDown className="w-4 h-4 mr-1" /> {t('reject')}
          </Button>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">{t('noReviewsFound')}</div>
          ) : (
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">{review.productId?.name || t('unknownProduct')}</p>
                          <p className="text-sm text-muted-foreground">
                            by {review.customerId?.name || t('unknown')} ({review.customerId?.email || ''})
                          </p>
                        </div>
                      </div>
