...
// Normalize API tree: rename `children` → `subcategories` recursively
const normalizeTree = (nodes) => {
  if (!Array.isArray(nodes)) return [];
  return nodes.map(node => {
    const kids = normalizeTree(node.children || node.subcategories || []);
...
export const CategoryProvider = ({ children }) => {
  ...
  const fetchCategories = async (force = false) => {
    ...
    try {
      ...
      const normalized = Array.isArray(data) && data.length > 0 ? normalizeTree(data) : null;
      const arr = normalized || FALLBACK_CATEGORIES;
      setCategories(arr);
      if (normalized) writeCache(arr);
      return arr;
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      categoryService.getPublicAll()
        .then(data => {
          const normalized = Array.isArray(data) && data.length > 0 ? normalizeTree(data) : null;
          if (normalized) { setCategories(normalized); writeCache(normalized); }
        })
        .catch(() => {});
    } else {
      fetchCategories();
